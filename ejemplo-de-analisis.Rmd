...
Puede ser que sea un poco inabarcable mostrar la matriz de comunidad, porque podría tener muchas especies (cada columna es una especie de la familia elegida), en cuyo caso te recomiendo, sólo para fines de exploración pero NO para incluir en el manuscrito, que selecciones columnas al azar (el código siguiente selecciona 10 columnas).
...
Para fines de exploración, puedes usar la alternativa `mc %>% View()` para representarla en el visor de RStudio. Pero recuerda: no intentes colocar tablas grandes en el manuscrito, porque desbordarán el PDF y no serán legibles. Si una tabla es importante para el manuscrito, pero ocupa mucho espacio, puedes considerar incluirla en la sección de "Información suplementaria" del mismo.

Esta es la lista de especies mostrada en forma de tabla (específicamente, usando una función personalizada basada en el paquete `kableExtra`).

```{r}
lista_especies <- data.frame(Especies = sort(names(mc)))
lista_especies %>%
...
```{r}
num_sitios_especies <- data.frame(
  `Número de sitios donde fue reportada cada especie` = sort(colSums(mc), decreasing = T),
           check.names = F) %>% 
  rownames_to_column('Especie')
num_sitios_especies %>%
  estilo_kable(
    titulo = paste('Número de sitios en los que está presente cada especie',
                   '(orden descendente por número de sitios)'), 
    nombres_filas = F, alinear = 'cr')
```

La tabla es grande, porque tiene tanta filas como especies. Un extracto podría ser útil en el manuscrito (siempre que lo cites dentro del mismo), por ejemplo, las primeras 10 filas ordenadas descendentemente por número de sitios.
...
num_sitios_especies %>% 
  slice_head(n = 10) %>%
  estilo_kable(
    titulo = paste('Número de sitios en los que estás presentes las 10 especies más comunes',
                   '(orden descendente por número de sitios)'), 
    nombres_filas = F, alinear = 'cr')
```


También se puede obtener la riqueza de especies por sitios.

```{r}
riqueza_por_sitios <- data.frame(`Riqueza por sitios` = rowSums(mc),
...
Al igual que en casos anteriores, esta tabla podría desbordar la salida, pues en este caso tiene muchas filas. Puedes incluir este tipo de tablas-resumen en tu manucrito siempre que lo consideres relevante en tu texto (tabla que se incluya en el manuscrito, tabla que necesita referencia en el texto). Un buen extracto podría ser seleccionar las primeras filas (sólo lo insertarías en tu manuscrito si lo refieres debidamente); por ejemplo, las primeras 10 filas ordenadas descendentemente en función de la riqueza por sitios, no parecen desbordar mucho el PDF en mi caso.

```{r}
riqueza_por_sitios %>% 
  slice_head(n = 10) %>%
  estilo_kable(
    titulo = 'Número de especies en los 10 sitios de mayor riqueza (orden descendente por riqueza)', 
    nombres_filas = F, alinear = 'cr')
```
...
```{r, fig.cap='Distribución de las especies según sitios', fig.width=10, fig.height=7, dpi=300}
grafico_mosaico <- crear_grafico_mosaico_de_mc(mc, tam_rotulo = 8) + xlab('Sitios') + ylab('Especie')
grafico_mosaico
```

Si los rótulos del gráfico se superpponen o son ilegibles, puedes probar a exportar el gráfico con este código, integrarlo donde sea conveniente (ver párrafos posteriores):

```{r}
jpeg('grafico_mosaico.jpg', width = 4000, height = 2200, res = 350)
grafico_mosaico
graphics.off()
```

Revisa el archivo `grafico_mosaico.jpg` y verifica si luce bien. Si luce bien, lo puedes llevar a tu manuscrito. Juega con los argumentos `width`, `height` y `res` para ajustar el tamaño y la resolución del gráfico.

> ¡¡¡IMPORTANTE!!! Si estás ejecutando código interactivamente, no olvides ejecutar `graphics.off()` después de ejecutar `jpeg...` y `grafico_mosaico`, pues de lo contrario, el dispositivo gráfico quedará abierto en el archivo JPEG creado, y no se mostrarán más gráficos a continuación en la ventana gráfica.

Puedes insertar tu figura de múltiples formas. En la PD01 expliqué al menos dos formas. Esta es una de las que más control te ofrece sobre el gráfico.

```{r grafico_mosaico, echo=F, fig.cap='Distribución de las especies (presencia/ausencia) según sitios'}
knitr::include_graphics('grafico_mosaico.jpg')
```

Si tu familia elegida tiene muchas especies, o las especies están presentes en muchos sitios, el gráfico de mosaico podría no ser legible aun cambiando la resolución, anchura y altura. En ese caso, luego de probar opciones, consúltame para que intentemos colocar debidamente en tu manuscrito.
...
Si notas que los elementos se empastan, prueba crear un JPEG conteniendo el mapa y ajusta el tamaño y la resolución del gráfico, tal como te mostré para el gráfico de mosaico. Si el mapa se ve bien, podrías usar el objeto `todos_los_objetos$aoi_reg_hex_inter_g` para llevarlo a tu manuscrito.
...
Al igual que la matriz de comunidad, la ambiental tiene muchas columnas y filas. E, igualmente, no te recomiendo que la incluyas en el documento de manuscrito. Puedes seleccionar columnas al azar o de manera específica usando el índice de columnas (por ejemplo, `colnames(env)`), o usar la alternativa `env %>% View()` para representarla en el visor de RStudio.
...
Se muestra a continuación un diagrama de cajas que sólo incluye un grupo de variables ambientales. Son muchas variables ambientales, y si se incluyen todas, el servidor no aguantaría o el gráfico no sería legible. Es por ello que ves el comando `select(matches('^ESA'))`, el cual selecciona las columnas que comienzan por la cadena de caracteres 'ESA'. El Sufijo de grupo "^ESA", que se refiere a los datos de cobertura del suelo de la Agencia Espacial Europea (ESA 2021, imágenes Sentinel 1 y Sentinel 2 a 10 m de resolucion máximo). Se deben hacer gráficos para otros grupos de variables, ver la lista de grupos posibles en la columna "Sufijo del grupo" de la Tabla 1 de [este HTML](https://geofis.github.io/zonal-statistics/README.html). Otros grupos posibles son `r vector_a_lista(grep('ESA', gsub(' .*', '', colnames(env)) %>% unique(), invert = T, value = T))`. ES IMPRESCINDIBLE que visites el enlace anterior por dos razones: 1) Para que te ayude a comprender mejor las múltiples variables disponibles y cómo fueron estas derivadas; 2) Para que puedas citar debidamente cada fuente usada, pues no deberías quedarte sólo en citar mi repo, sino también, las fuentes originales.
...
Es probable que el árbol anterior no sea legible, o que se produzca un error si estás ejecutando código en la consola. En ese caso, te recomiendo que intentes guardar el árbol como archivo JPEG, y así lo podrás ver con claridad. Esto se haría de la misma forma que hice con el gráfico de mosaicos:

```{r}
jpeg('arboles_todos.jpg', width = 4000, height = 2200, res = 350)
par(mfrow = c(2,2))
invisible(
  map(names(lista_cl),
      function(x)
        plot(lista_cl[[x]], main = paste0(x, '\n(árbol de evaluación)'), hang = -1)))
graphics.off()
```

Para insertarlo, uso este bloque de código (nota que me refiero a Acanthaceae en el título o "*caption*", pero debes editar):

```{r arboles_todos, echo=F, fig.cap='Agrupamiento jerárquico de sitios (hexágonos H3) en función de su riqueza de especies de Acanthaceae'}
knitr::include_graphics('arboles_todos.jpg')
```


A continuación, **calcularé la distancia y la correlación cofenéticas; esta última, la correlación cofenética, se emplea como criterio flexible para elegir el método de agrupamiento idóneo, pero no debe usarse de manera estricta**. Se supone que el método con la mayor correlación cofenética explica mejor el agrupamiento de la comunidad. Si quieres comprender mejor esta técnica, consulta el vídeo que te referí en el párrafo anterior, así como los libros de referencia. Normalmente, el método UPGMA obtiene la mayor correlación cofenética, pero esto se debe a que su procedimiento de obtención maximiza precisamente dicha métrica. No es recomendable conservar un único método de agrupamiento, normalmente es bueno usar al menos dos. Ward es muchas veces recomendado como método de contraste, por basarse en procedimientos de cálculo muy distintos a los de UPGMA.
...
Ahora, calcularé **las anchuras de siluetas, una métrica que ayuda a determinar en cuántos grupos se organiza la comunidad; las anchuras de silueta no deben usarse como método estricto, y sólo debe usarse de forma flexible para informarnos sobre el número máximo de grupos posibles**. Considera las siguientes reglas:
...
### Anchuras de siluetas para el método UPGMA
...
### Anchuras de siluetas para el método Ward
...
Observa cómo se comporta el agrupamiento con otros métodos de evaluación, como el remuestreo por *bootstrap* multiescalar.
...
> Nota: al igual que en casos anteriores, si el gráfico es ilegible, opta por convertirlo a JPEG.
...
Apliquemos el análisis de agrupamiento a la matriz ambiental. La idea central en este punto es que, si la matriz ambiental presenta patrones parecidos a los de la matriz de comunidad, significa que el agrupamiento utilizado hace sentido entre ambos conjuntos de datos (comunidad y hábitat) de forma consistente. Si ambos conjuntos de datos son consistentes, significa que existe algún grado de asociación, aunque sea sólo una mera asociación estadística. Por extensión, el hecho de que ambas matrices presenten patrones parecidos, significa que una podría tener poder explicativo sobre la otra (y viceversea), es decir, que las variables ambientales podrían ayudar a explicar la distribución de las especies de la familia elegida.

Agruparé los sitios de muestreo de la matriz ambiental según los grupos previamente definidos.
...
Interesan especialmente, aquellas variables que presentan efecto entre los grupos. En concreto, aquellas en las que la prueba de Kruskal-Wallis resulta significativa, pues esta prueba es más robusta que la ANOVA en ausencia normalidad y homocedasticidad. Se puede filtrar el resultado de la tabla anterior para mostrar sólo aquellas variables en las que la prueba de Kruskal-Wallis resulta significativa (nota que en la tabla anterior también puedes filtrar de forma interactiva).


```{r}
env_grupos_ak %>% 
  filter(p_valor_k < umbral_alfa) %>%
  datatable(options = list(pageLength = 140, autoWidth = FALSE),
            filter = 'top',) %>% 
  formatSignif(columns = "p_valor_a", digits = 1) %>%
  formatSignif(columns = "p_valor_k", digits = 1)
...
En el ejemplo analizado, nota que las variables que muestran efecto son bioclimáticas (CH-BIO, WCL), horas de insolación, distancia a carretera (¿sesgo de muestreo?), algunas variables de cobertura del suelo (ESA, CGL) y otras de topografía/geomorfología (G90).


Explora tus resultados también gráficamente, usando un panel de diagramas de cajas.
...
env_grupos_p <- env_grupos %>% 
  ...
  facet_wrap(
    ...
    labeller = labeller(
      variable = function(x)
        str_trunc(x, width = 45, ellipsis = '...')))
env_grupos_p
```

El gráfico anterior, seguramente será ilegible en la pestaña de gráficos de RStudio, pero puedes hacerlo legible exportándolo como JPEG (los nombres de cada *facet* corresponde a cada una de las 140 variables, pero están truncados a partir del caracter 45; para ver los nombres completos, usa `unique(env_grupos$variable)` en la consola de R; el significado de cada variable se encuentra en el [repo fuente](https://geofis.github.io/zonal-statistics/README.html)):

```{r}
jpeg('env_grupos_p.jpg', width = 3000, height = 4000, res = 100)
print(env_grupos_p)
graphics.off()
```
...
```{r env_grupos_p, echo=F, fig.cap='Variables ambientales de República Dominicana según agrupamiento de sitios en función de su riqueza de especies de Acanthaceae'}
knitr::include_graphics('env_grupos_p.jpg')
```

Es útil representar, cartográficamente, los grupos de hexágonos en función de su riqueza de especies de la familia eleida, en este caso, Acanthaceae. Este código lo muestra.

```{r}
# Mapa
todos_los_objetos$aoi_reg_hex_inter_g$layers[[2]]$data %>% left_join(
  env_grupos %>%
    select(h3_address = sitios_de_muestreo, grupos) %>%
    as.data.frame()) %>%
  ggplot() +
  aes(fill = grupos) +
  geom_sf(data = todos_los_objetos$aoi_reg_hex_inter_g$data, fill = NA) +
  geom_sf() +
  scale_fill_brewer(palette = 'Set1') +
  theme_bw()
```
...
El gráfico, casi seguramente, es ilegible. Recuerda que puedes exportarlo como JPEG usando cualquiera de los casos de uso anteriores.
...
```{r, include=F}
# Necesitamos que las cadenas sean de tipo factor para que el código siguiente funcione bien
...
Mapas representando la riqueza (N0) y los índices H y Hb2

```{r}
# Mapa para N0
mapa_para_n0 <- todos_los_objetos$aoi_reg_hex_inter_g$layers[[2]]$data %>% left_join(
  indices %>%
    select(h3_address = sitio, N0) %>%
    as.data.frame()) %>%
  ggplot() +
  aes(fill = N0) +
  geom_sf(data = todos_los_objetos$aoi_reg_hex_inter_g$data, fill = NA) +
...
```{r}
# Mapa para H
mapa_para_h <- todos_los_objetos$aoi_reg_hex_inter_g$layers[[2]]$data %>% left_join(
  indices %>%
    select(h3_address = sitio, H) %>%
    as.data.frame()) %>%
  ggplot() +
  aes(fill = H) +
  geom_sf(data = todos_los_objetos$aoi_reg_hex_inter_g$data, fill = NA) +
...
```{r}
# Mapa para Hb2
mapa_para_hb2 <- todos_los_objetos$aoi_reg_hex_inter_g$layers[[2]]$data %>% left_join(
  indices %>%
    select(h3_address = sitio, Hb2) %>%
    as.data.frame()) %>%
  ggplot() +
  aes(fill = Hb2) +
  geom_sf(data = todos_los_objetos$aoi_reg_hex_inter_g$data, fill = NA) +
...
```{r, out.width='100%'}
panel_cor_riq_ch_bio <- bind_cols(indices %>% select(N0), env %>%
            select(matches('^CH-BIO')) %>% 
            rename_with(.fn = ~ paste0('AMB_', .))) %>%
  ggpairs(
    labeller = label_wrap_gen(width=10),
    upper = list(continuous = GGally::wrap("cor", size = 3))) +
  theme(text = element_text(size = 10))
panel_cor_riq_ch_bio
```

Nota que el panel de correlación anterior sólo incluye un grupo de variables ambientales. Son muchas variables ambientales, y si se incluyen todas, el servidor no aguantaría. Es por ello que ves el comando `select(matches('^CH-BIO'))`, el cual selecciona las columnas que comienzan por la cadena de caracteres 'CH-BIO'. 'CH-BIO' se refiere a variables bioclimáticas del dataset CHELSA, 'WCL' son datos bioclimáticos del dataset WorldClim, 'ESA' se refiere a los datos de cobertura del suelo de la Agencia Espacial Europea (ESA 2021, imágenes Sentinel 1 y Sentinel 2 a 10 m de resolucion máximo), y así sucesivamente. Se deben hacer más paneles de correlación para otros grupos de variables, ver la lista de grupos posibles en la columna "Sufijo del grupo" de la Tabla 1 de [este HTML](https://geofis.github.io/zonal-statistics/README.html). Otros grupos posibles: CGL, G90, WCL, entre muchos otros. Tte convendrá elegir grupos donde hayas visto que las variables ambientales presentan efectos entre grupos, a partir de los resultados del análisis de Kruskal-Walis. Y como siempre, puedes exportar el panel como JPEG para mejorar su legibilidad.
...
riqueza_estimaciones <- data.frame(specpool(mc) %>% select(-matches('.se$'))) %>% 
  ...
  mutate(`Cobertura (%)` = (filter(., Variable == "Riqueza observada") %>% pull(Valor)) / Valor * 100) %>% 
...
Curva de acumulación de especies. La curva de acumulación de especies es una representación gráfica de la riqueza de especies en función del número de sitios muestreados. La curva de acumulación de especies es una herramienta útil para evaluar la eficacia de muestreo y la completitud de inventarios de biodiversidad. En general, se considera que una curva de acumulación de especies que se estabiliza indica que el muestreo ha sido suficiente para capturar la mayoría de las especies presentes en un área. En cambio, una curva que no se estabiliza sugiere que se necesitan más esfuerzos de muestreo para capturar la diversidad total de especies presentes en un área.

A continuación, el análisis de estimación de riqueza de especies en la comunidad con índices mejorados de Chao.
...
# Si aparece, sólo comentar que entiendo que el problema
...
# No es un análisis crítico, pero si lo necesitas, puedes usar la aplicación Shiny.
```
