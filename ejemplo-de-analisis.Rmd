...
Si los rótulos del gráfico se superpponen o son ilegibles, puedes probar a exportar el gráfico con este código:

```{r}
jpeg('grafico_mosaico.jpg', width = 4000, height = 3000, res = 300)
grafico_mosaico
graphics.off()
```

Revisa el archivo `grafico_mosaico.jpg` y verifica si luce bien. Si luce bien, lo puedes llevar a tu manuscrito. ¡¡¡IMPORTANTE!!! Si estás ejecutando código interactivamente, no olvides ejecutar `graphics.off()` después de ejecutar `jpeg...` y `grafico_mosaico`, pues de lo contrario, el dispositivo gráfico quedará abierto en el archivo JPEG crado, y no se mostrarán más gráficos a continuación en la ventana gráfica.
...
Se muestra a continuación un diagrama de cajas que sólo incluye un grupo de variables ambientales. Son muchas variables ambientales, y si se incluyen todas, el servidor no aguantaría o el gráfico no sería legible. Es por ello que ves el comando `select(matches('^ESA'))`, el cual selecciona las columnas que comienzan por la cadena de caracteres 'ESA'. El Sufijo de grupo "^ESA", que se refiere a los datos de cobertura del suelo de la Agencia Espacial Europea (ESA 2021, imágenes Sentinel 1 y Sentinel 2 a 10 m de resolucion máximo). Se deben hacer gráficos para otros grupos de variables, ver la lista de grupos posibles en la columna "Sufijo del grupo" de la Tabla 1 de [este HTML](https://geofis.github.io/zonal-statistics/README.html). Otros grupos posibles: CH-BIO, CGL, G90, WCL, entre muchos otros.
...
Nota que el panel de correlación anterior sólo incluye un grupo de variables ambientales. Son muchas variables ambientales, y si se incluyen todas, el servidor no aguantaría. Es por ello que ves el comando `select(matches('^ESA'))`, el cual selecciona las columnas que comienzan por la cadena de caracteres 'ESA'. Como bien explique arriba, 'ESA' se refiere a los datos de cobertura del suelo de la Agencia Espacial Europea (ESA 2021, imágenes Sentinel 1 y Sentinel 2 a 10 m de resolucion máximo). Se deben hacer más paneles de correlación para otros grupos de variables, ver la lista de grupos posibles en la columna "Sufijo del grupo" de la Tabla 1 de [este HTML](https://geofis.github.io/zonal-statistics/README.html). Otros grupos posibles: CH-BIO, CGL, G90, WCL, entre muchos otros.
